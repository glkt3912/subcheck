...
export default function Home() {
  ...
  const [showReturningUserSection, setShowReturningUserSection] = useState(false);
      ...
      try {
        ...
        if (history.length > 0) {
          ...
          const daysSinceLastDiagnosis = Math.floor(
            (Date.now() - new Date(lastDiagnosis.createdAt).getTime()) / (1000 * 60 * 60 * 24)
          );
          ...
          if (daysSinceLastDiagnosis >= 7) {
            alerts.push({
              ...
              actions: [
                { type: 'navigate', label: '新しい診断を開始', url: '/diagnosis/select' },
          ...
          if (lastDiagnosis.wasteRate > 50) {
            alerts.push({
              ...
              actions: [
                { type: 'navigate', label: '前回結果を確認', url: '/diagnosis/results' },
                { type: 'navigate', label: '新しい診断', url: '/diagnosis/select' },
          ...
          if (daysSinceLastDiagnosis >= 30) {
            alerts.push({
              ...
              severity: 'info',
              title: '月次見直しのタイミングです',
              message: '新しいサービスの追加や料金変更がある可能性があります。包括的な見直しを行いましょう。',
              actions: [
                { type: 'navigate', label: '包括的診断を開始', url: '/diagnosis/select' },
