...
import { DiagnosisResult, AlertNotification } from '@/types';
  ...
  describe('generateAlerts', () => {
    it('should generate high waste rate alert for 70% waste rate', () => {
      const diagnosisResult: DiagnosisResult = {
        subscriptions: [
          { subscriptionId: 'netflix', usageFrequency: 'unused', isCustom: false, dateAdded: '2024-01-01' },
          { subscriptionId: 'spotify', usageFrequency: 'monthly', isCustom: false, dateAdded: '2024-01-01' }
        ],
      ...
      const wasteRateAlert = alerts.find(alert => alert.type === 'waste_rate');
    ...
    it('should generate unused service alert', () => {
      const diagnosisResult: DiagnosisResult = {
        subscriptions: [
          { subscriptionId: 'netflix', usageFrequency: 'unused', isCustom: false, dateAdded: '2024-01-01' },
          { subscriptionId: 'spotify', usageFrequency: 'unused', isCustom: false, dateAdded: '2024-01-01' }
        ],
      ...
      const unusedAlert = alerts.find(alert => alert.type === 'unused_service');
    ...
    it('should not generate alerts for normal usage', () => {
      const diagnosisResult: DiagnosisResult = {
        subscriptions: [
          { subscriptionId: 'netflix', usageFrequency: 'daily', isCustom: false, dateAdded: '2024-01-01' },
          { subscriptionId: 'spotify', usageFrequency: 'daily', isCustom: false, dateAdded: '2024-01-01' }
        ],
    ...
    it('should generate budget exceeded alert', () => {
      ...
      localStorageMock.getItem.mockReturnValue(JSON.stringify({
        enabled: true,
        conditions: [
          { id: 'budget-exceeded', type: 'budget_exceeded', enabled: true, threshold: 10000, userConfigured: false }
        ],
        notificationPreferences: { showInApp: true, showBrowser: false, frequency: 'immediate' },
        lastUpdated: new Date()
      }));

      const alerts = AlertService.generateAlerts(diagnosisResult);

      const budgetAlert = alerts.find(alert => alert.type === 'budget_exceeded');
  ...
  describe('getAlertSettings', () => {
    ...
    it('should merge user settings with defaults', () => {
      localStorageMock.getItem.mockReturnValue(JSON.stringify({
        enabled: false,
        conditions: [
          { id: 'waste-rate-high', type: 'waste_rate', enabled: false, threshold: 80, userConfigured: true }
        ],
        notificationPreferences: { showInApp: false, showBrowser: true, frequency: 'weekly' },
        lastUpdated: new Date()
      }));
      ...
      const wasteRateCondition = settings.conditions.find(c => c.id === 'waste-rate-high');
  ...
  describe('saveAlertSettings', () => {
    it('should save settings to localStorage', () => {
      const settings = {
        enabled: true,
        conditions: [],
        notificationPreferences: { showInApp: true, showBrowser: false, frequency: 'immediate' as const },
        lastUpdated: new Date()
      };
